...
#define TX_JIT_DELAY            150000   /* Pre-delay to program packet for TX in microseconds */
...
bool jit_queue_is_full( jit_queue_t * queue )
{
    bool result;
...
bool jit_queue_is_empty( jit_queue_t * queue )
{
    bool result;
...
void jit_queue_init( jit_queue_t * queue )
{
    int i;
...
{
    jit_node_t *p = (jit_node_t *)a;
    jit_node_t *q = (jit_node_t *)b;
    int *counter = (int *)arg;
    int p_count, q_count;
...
void jit_sort_queue( jit_queue_t * queue )
{
    int counter = 0;
...
jit_error_t jit_enqueue( jit_queue_t * queue, struct timeval *host, uint8_t brd, sx1301ar_tx_pkt_t * packet, jit_pkt_type_t pkt_type )
{
    int i = 0;
    uint32_t time_us = sx1301ar_host2cnt(host, 0);
    uint32_t packet_post_delay = 0;
    uint32_t packet_pre_delay = 0;
    uint32_t target_pre_delay = 0;
    jit_error_t err_collision;
    ...
    switch( pkt_type )
    {
        ...
        case JIT_PKT_TYPE_DOWNLINK_CLASS_C:
            ...
            packet_post_delay = (uint32_t)(sx1301ar_tx_pkt_time_on_air( 0, packet, NULL, NULL, NULL )) * 1000UL; /* in us */
            break;
    ...
    queue->nodes[queue->num_pkt].brd = brd;
    memcpy( &(queue->nodes[queue->num_pkt].pkt), packet, sizeof(sx1301ar_tx_pkt_t) );
    ...
    MSG_DEBUG( DEBUG_JIT, "[%llu] enqueued packet with count_us=%u (size=%u bytes, toa=%u us, type=%u)\n", sx1301ar_timestamp(), packet->count_us, packet->size, packet_post_delay, pkt_type );
...
jit_error_t jit_dequeue( jit_queue_t * queue, int index, uint8_t * brd, sx1301ar_tx_pkt_t * packet, jit_pkt_type_t * pkt_type )
{
    if( (packet == NULL) || (brd == NULL) || (pkt_type == NULL) )
    {
    ...
    *brd = queue->nodes[index].brd;
    memcpy( packet, &(queue->nodes[index].pkt), sizeof(sx1301ar_tx_pkt_t) );
    queue->num_pkt--;
    *pkt_type = queue->nodes[index].pkt_type;
    ...
    memcpy( &(queue->nodes[index]), &(queue->nodes[queue->num_pkt]), sizeof(jit_node_t) );
    memset( &(queue->nodes[queue->num_pkt]), 0, sizeof(jit_node_t) );
    ...
    MSG_DEBUG( DEBUG_JIT, "[%llu] dequeued packet with count_us=%u from index %d\n", sx1301ar_timestamp(), packet->count_us, index );
...
jit_error_t jit_peek( jit_queue_t * queue, struct timeval *host, int * pkt_idx )
{
    ...
    time_us = sx1301ar_host2cnt(host, 0);
    ...
    for( i = 0; i < queue->num_pkt; i++ )
    {
        ...
        if( (queue->nodes[i].pkt.count_us - time_us) >= TX_MAX_ADVANCE_DELAY )
        {
            ...
            memcpy( &(queue->nodes[i]), &(queue->nodes[queue->num_pkt]), sizeof(jit_node_t) );
            memset( &(queue->nodes[queue->num_pkt]), 0, sizeof(jit_node_t) );
    ...
    if( (queue->nodes[idx_highest_priority].pkt.count_us - time_us) < TX_JIT_DELAY )
    {
        *pkt_idx = idx_highest_priority;
        MSG_DEBUG( DEBUG_JIT, "[%llu] peeked packet with count_us=%u at index %d\n", sx1301ar_timestamp(), queue->nodes[idx_highest_priority].pkt.count_us, idx_highest_priority );
    }
    else
    {
        *pkt_idx = -1;
...
void jit_print_queue( jit_queue_t * queue, bool show_all, int debug_level )
{
    int i = 0;
    int loop_end;
    ...
    else
    {
        ...
        {
            MSG_DEBUG( debug_level, " - node[%d]: count_us=%u - type=%d - brd=%u\n",
                        i,
